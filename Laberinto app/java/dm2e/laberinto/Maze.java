...
public class Maze {
     ...
     */
    public Maze(Context contexto, Point[][] coord, int nX, int nY) {
    ...
    public Maze() {
        this(null/*, new Point[Utils.MAX][Utils.MAX], -1, -1*/);
    }
    ...
    public Point getNeighbor(Point p, Movements strat) {
        if (p == null || strat != Movements.RIGHT && strat != Movements.UP && strat != Movements.LEFT && strat != Movements.DOWN) {
            Log.e(TAG, "ERROR getNeighbor: " + p + ", " + strat);
            return null;
        }
    ...
    public Point getNeighbor(int x, int y, Movements strat) {
        return getNeighbor(getPoint(x, y), strat);
    }
    ...
    public boolean setPoint(Point p) {
        if (p == null) {
            Log.e("Maze.java", "error en setPoint 1");
            return false;
        }
        int x = p.getX(), y = p.getY();
    ...
    public String toString() {
        ...
        for (int y = 0; y < getNrows(); txt.append("\n"), y++) {
            for (int x = 0; x < getNcols(); x++) {
                try {
    ...
    public String toCoordString() {
        ...
        for (int y = 0; y < getNrows(); txt.append("\n"), y++) {
            for (int x = 0; x < getNcols(); x++) {
                try {
                    txt.append(String.format("%s", getPoint(x, y)));
                } catch (Exception e) {
                    txt.append(String.format("[(%2d, %2d) %s]", y, x, FileChars.ERRORCHAR.c));
                }
            }
        }
        return txt.toString();
...
//    public boolean read0(@RawRes int pf) { try { List<String> lineas = new ArrayList<>();InputStream is = this.context.getApplicationContext().getResources().openRawResource(pf);Scanner sc = new Scanner(is);if (sc.hasNextLine()) { while (sc.hasNextLine()) { lineas.add(sc.nextLine()); } }sc.close();is.close();if (lineas.isEmpty()) { throw new Exception("Error in Maze.read.Files.readAllLines()"); }/* creamos punto que se utiliza como buffer */Point temp = new Point();if (temp == null) { throw new Exception("Error in read.temp = new Point()");}/* asignamos dimensiones al laberinto */String[] nXY = lineas.get(0).split(" ");int nI = Integer.parseInt(nXY[0]), nJ = Integer.parseInt(nXY[1]);if (!setSize(nJ, nI)) { throw new Exception("Error in read.setSize()"); }/* leemos el fichero linea a linea */for (int x = 0; x < nJ; x++) { char[] buff = lineas.get(x + 1).toCharArray();for (int y = 0; y < nI; y++) { char c = buff[y];/* ajustamos los atributos del punto leido (falta añadir control de errores) */if (!temp.setX(x)) { Log.e(TAG, "Error in read.temp.setX(" + x + ")"); }if (!temp.setY(y)) { Log.e(TAG, "Error in read.temp.setY(" + x + ")"); }if (!temp.setSymbol(c)) { Log.e(TAG, "Error in read.temp.setSymbol(" + c + ")"); }/* insertamos el punto en el laberinto (falta añadir control de errores) */if (!setPoint(temp)) { Log.e(TAG, "Error in read.temp.setPoint(" + temp + ")"); } } }return true; } catch (Exception e) {/*Log.e(TAG,e.toString());*/e.printStackTrace(); return false;} }
    ...
    public Stack<Point> toStack() {
        ...
        for (int y = 0; y < getNrows(); y++) {
            for (int x = 0; x < getNcols(); x++) {
                if (stack.size() == getNrows() * getNcols()) {
                    Log.e(TAG, "Stack is too small for maze.");
                    return null;
                }
                Point cp = getPoint(x, y);
                if (stack.push(cp) == null) {
                    Log.e(TAG, "Error while adding point to stack.");
                    return null;
                }
            }
        }
    ...
    public Queue<Point> toQueue() {
        ...
        for (int y = 0; y < getNrows(); y++) {
            for (int x = 0; x < getNcols(); x++) {
                if (queue.size() == getNrows() * getNcols()) {
                    Log.e(TAG, "Queue is too small for maze.");
                    return null;
                }
                Point cp = getPoint(x, y);
                if (!queue.add(cp)) {
                    Log.e(TAG, "Error while adding point to queue.");
                    return null;
                }
            }
        }
    ...
    public void startMazeText() {
        ...
        while (!actual.equals(output)) {
            ...
            try {
                ...
                switch (move) {
                    case 'w': {
                        neighbor = getNeighbor(actual, Movements.UP);
                        break;
                    }
                    case 'a': {
                        neighbor = getNeighbor(actual, Movements.LEFT);
                        break;
                    }
                    case 's': {
                        neighbor = getNeighbor(actual, Movements.DOWN);
                        break;
                    }
                    case 'd': {
                        neighbor = getNeighbor(actual, Movements.RIGHT);
                        break;
                    }
                    default: {
     ...
     * @param screenHeight    The screen height
     */
    public void setDrawValues(Bitmap[] bitmaps, float xPointsOnScreen, float yPointsOnScreen, float screenWidth, float screenHeight) {
...
//    public String printMazeBorder(Point p, int cellsX, int cellsY) { int pX = (p != null) ? p.getX() : -1, pY = (p != null) ? p.getY() : -1;StringBuilder txt = new StringBuilder();for (int y = -1; y <= getNrows(); txt.append((y + 1 <= getNrows()) ? "\n" : ""), y++) { for (int x = -1; x <= getNcols(); x++) { try { if (x == pX && y == pY) txt.append(Movements.ACTUAL.c);else if (x == -1 || y == -1) txt.append('#');else if (x == getNcols() || y == getNrows()) txt.append('%');else txt.append(String.format("%c", getPoint(x, y).getSymbol())); } catch (Exception e) { Log.e(TAG, String.format("[%2d, %2d] en [%2d, %2d)", y, x, getNcols(), getNrows())); } } }return txt.toString(); }
        ...
        for (int y = 0, nY = 0, flag = 0; y < getNrows() && nY < cellsY; y++) {
            ...
            for (int x = 0, nX = 0; x < getNcols() && nX < cellsX; x++) {
                if (x == xCoord && y == yCoord) {
                    try {
                        if (x == pX && y == pY) txt.append(Movements.ACTUAL.c);
                        else if (getPoint(x, y).getSymbol() == FileChars.SPACE.c) txt.append('·');
                        else txt.append(String.format("%c", getPoint(x, y).getSymbol()));
                    } catch (Exception e) {
            ...
            if (flag == 1) {
                txt.append((y + 1 < getNrows() && nY + 1 < cellsY) ? "\n" : "");
                yCoord++;
                nY++;
            }
        }
    ...
    public Bitmap getBitmap(int x, int y) {
        char c = FileChars.ERRORCHAR.c;
        if (getPoint(x, y) != null) c = getPoint(x, y).getSymbol();
